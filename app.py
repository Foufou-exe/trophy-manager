...
def updateGameSearch():
    if request.args.get('update') != "":
        ...
        if request.args.get('affichage') == "1":
            ...
            return render_template('show.html', game=game, str=str, affichage=1, bool=bool)
        elif request.args.get('affichage') == "0":
            game = showGameSelected(request.args.get('GameUID'))
            return render_template('show.html', game=game, str=str, affichage=0, bool=bool)
        else:
            game = showGameSelected(request.args.get('GameUID'))
            return render_template('show.html', game=game, str=str, affichage=2, bool=bool)
    else:
        return render_template('returnMainMenu.html')
