...
def addMainTrophee(
    GameUID, Nom, Description, Trophee, Pourcentage, Image, id, Terminer
):
    conn = sqlite3.connect("TropheeBdd.db")
    cursor = conn.cursor()
    ...
    conn.commit()
    print(
        "Record inserted successfully into GameRegisterMainTrophee table ",
        cursor.rowcount,
    )
    cursor.close()
...
def removeMainTropheeFromID(id):
    ...
    conn.commit()
    print(
        "Record removed successfully from GameRegisterMainTrophee table ",
        cursor.rowcount,
    )
    cursor.close()
...
def updateMainTropheeFromID(terminer, id):
    ...
    conn.commit()
    print(
        "Record update successfully from GameRegisterMainTrophee table ",
        cursor.rowcount,
    )
    cursor.close()
...
def updateMainTropheeFromGameUID(terminer, GameUID):
    ...
    cursor = conn.cursor()
    sqlite_update_query = (
        "UPDATE GameRegisterMainTrophee SET Terminer = ? WHERE GameUID = ?"
    )
...
def getMainTropheeFromID(GameUID, id) -> dict:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM GameRegisterMainTrophee WHERE id = ? AND GameUID = ?",
        (id, GameUID),
    )
    rows = cur.fetchone()
    if type(rows) is not type(None):
...
def addDLCTrophee(
    GameUID, numDLC, Nom, Description, Trophee, Pourcentage, Image, id, Terminer
):
    conn = sqlite3.connect("TropheeBdd.db")
    cursor = conn.cursor()
    ...
    conn.commit()
    print(
        "Record inserted successfully into GameRegisterDLCTrophee table ",
        cursor.rowcount,
    )
    cursor.close()
...
def removeDLCTropheeFromID(id):
    ...
    conn.commit()
    print(
        "Record removed successfully from GameRegisterDLCTrophee table ",
        cursor.rowcount,
    )
    cursor.close()
...
def updateDLCTropheeFromID(terminer, id):
    ...
    conn.commit()
    print(
        "Record update successfully from GameRegisterDLCTrophee table ", cursor.rowcount
    )
    cursor.close()
...
def updateDLCTropheeFromGameUID(terminer, GameUID):
    ...
    cursor = conn.cursor()
    sqlite_update_query = (
        "UPDATE GameRegisterDLCTrophee SET Terminer = ? WHERE GameUID = ?"
    )
    cursor.execute(sqlite_update_query, (terminer, GameUID))
    conn.commit()
    print(
...
def updateDLCTropheeFromNumDLC(terminer, numDLC):
    ...
    cursor = conn.cursor()
    sqlite_update_query = (
        "UPDATE GameRegisterDLCTrophee SET Terminer = ? WHERE numDLC = ?"
    )
...
def getAllDLCTropheeFromNumDLC(GameUID, numDLC) -> list:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM GameRegisterDLCTrophee WHERE GameUID = ? AND numDLC = ?",
        (GameUID, numDLC),
    )
    rows = cur.fetchall()
    listGame = []
...
def getDLCTropheeFromID(GameUID, id) -> dict:
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT * FROM GameRegisterDLCTrophee WHERE id = ? AND GameUID = ?",
        (id, GameUID),
    )
    rows = cur.fetchone()
    if type(rows) is not type(None):
