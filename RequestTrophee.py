def getGames(
    console="ps4", order="proche", direction="ASC", qquery="", letters="", page=1
) -> list:
    """
    This function returns a list containing information about games based on the search criteria provided.

    Args:
        console (str): The console to search for. Default is "ps4".
        order (str): The order to sort the results by. Default is "proche".
        direction (str): The direction to sort the results in. Default is "ASC".
        qquery (str): The name of the game to search for. Default is an empty string.
        letters (str): The first letter(s) of the game to search for. Default is an empty string.
        page (int): The page number of the search results to return. Default is 1.

    Returns:
        list: A list containing information about the games found. The list contains the following elements:
        [title, ps plus availability, number of DLCs, console, VR availability, impossible platinum, number of platinum trophies, number of gold trophies, number of silver trophies, number of bronze trophies, community rating out of 20, difficulty out of 10, cover image URL, game URL]
    """
...
def getGames(
    ...
    for game in allGames:
        ...
        # Ajout du ps extra
        indexExtra = [
            startHeadSectionDiv.index(index)
            for index in startHeadSectionDiv
            if "Extra" in index
        ]
        if len(indexExtra) > 0:
            gameFind["Extra"] = 1
        ...
        if len(indexDLC) > 0:
            gameFind["DLC"] = int(
                str(startHeadSectionDiv[indexDLC[0]])
                .split(">")[1]
                .split("<")[0]
                .replace(" DLC", "")
            )
        ...
        if len(startHeadSectionDiv) > 1:
            ...
            gameFind["console"] = (
                str(startHeadSectionDiv[0]).split(">")[1].split("<")[0]
            )
        startHeadSectionDiv = []
        # Ajout VR jeux
        indexVR = [
            startHeadSectionImg.index(index)
            for index in startHeadSectionImg
            if "VR" in str(index)
        ]
        if len(indexVR) > 0:
            gameFind["VR"] = 1
        ...
        # trophee platine
        indexPlatine = [
            bottomHeadSectionDiv.index(index)
            for index in bottomHeadSectionDiv
            if "platine" in str(index)
        ]
        gameFind["TropheePlatine"] = int(
            str(bottomHeadSectionDiv[indexPlatine[0]]).split(">")[1].split("<")[0]
        )
        # trophee Gold
        indexGold = [
            bottomHeadSectionDiv.index(index)
            for index in bottomHeadSectionDiv
            if "gold" in str(index)
        ]
        gameFind["TropheeGold"] = int(
            str(bottomHeadSectionDiv[indexGold[0]]).split(">")[1].split("<")[0]
        )
        # trophee Silver
        indexSilver = [
            bottomHeadSectionDiv.index(index)
            for index in bottomHeadSectionDiv
            if "silver" in str(index)
        ]
        gameFind["TropheeSilver"] = int(
            str(bottomHeadSectionDiv[indexSilver[0]]).split(">")[1].split("<")[0]
        )
        # trophee Bronze
        indexBronze = [
            bottomHeadSectionDiv.index(index)
            for index in bottomHeadSectionDiv
            if "bronze" in str(index)
        ]
        gameFind["TropheeBronze"] = int(
            str(bottomHeadSectionDiv[indexBronze[0]]).split(">")[1].split("<")[0]
        )
        # Note du jeu
        testNote = [
            bottomHeadSectionA.index(index)
            for index in bottomHeadSectionA
            if "test-note" in str(index)
        ]
        if len(testNote) > 0:
            gameFind["NoteJeu"] = float(
                str(bottomHeadSectionA[testNote[0]])
                .replace("\n", "")
                .split(">")[1]
                .split("<")[0]
            )
        ...
        dificulte = [
            bottomHeadSectionA.index(index)
            for index in bottomHeadSectionA
            if "guide-note" in str(index)
        ]
        if len(dificulte) > 0:
            gameFind["Difficulte"] = float(
                str(bottomHeadSectionA[dificulte[0]])
                .replace("\n", "")
                .split(">")[1]
                .split("<")[0]
            )
        ...
        gameFind["URL"] = (
            "https://www.psthc.fr"
            + str(URLHead).split(">")[0].split("?")[0].split('href="')[1]
        )
        # url pochette
...
def getGame(url) -> dict:
    ...
    Game["pochette"] = (
        str(startHead.find("div", attrs={"class": "game-picture-container"}))
        .split("url('")[1]
        .split("')")[0]
    )
    # Ajout de l'URL
    ...
    if type(startHeadGameCenterTrophee.find("div", attrs={"class": "platine"})) != type(
        None
    ):
        Game["TropheePlatine"] = (
            str(startHeadGameCenterTrophee.find("div", attrs={"class": "platine"}))
            .split(">")[1]
            .split("<")[0]
        )
    else:
    ...
    if type(startHeadGameCenterTrophee.find("div", attrs={"class": "gold"})) != type(
        None
    ):
        Game["TropheeGold"] = (
            str(startHeadGameCenterTrophee.find("div", attrs={"class": "gold"}))
            .split(">")[1]
            .split("<")[0]
        )
    else:
    ...
    if type(startHeadGameCenterTrophee.find("div", attrs={"class": "silver"})) != type(
        None
    ):
        Game["TropheeSilver"] = (
            str(startHeadGameCenterTrophee.find("div", attrs={"class": "silver"}))
            .split(">")[1]
            .split("<")[0]
        )
    else:
    ...
    if type(startHeadGameCenterTrophee.find("div", attrs={"class": "bronze"})) != type(
        None
    ):
        Game["TropheeBronze"] = (
            str(startHeadGameCenterTrophee.find("div", attrs={"class": "bronze"}))
            .split(">")[1]
            .split("<")[0]
        )
    else:
    ...
    # Ajout des VR
    indexVR = [
        startHeadGameCenterImg.index(index)
        for index in startHeadGameCenterImg
        if "VR" in str(index)
    ]
    if len(indexVR) > 0:
        Game["VR"] = 1
    ...
    try:
        Game["Difficulte"] = float(
            str(startHeadGameNote.find("div", attrs={"class": "guide-note"}))
            .split(">")[1]
            .split("<")[0]
        )
    except:
    ...
    try:
        Game["NoteJeu"] = float(
            str(startHeadGameNote.find("div", attrs={"class": "test-note"}))
            .split(">")[1]
            .split("<")[0]
        )
    except:
    ...
    sortie = [
        middleHeadLeftBR.index(index)
        for index in middleHeadLeftBR
        if "Date de sortie" in str(index)
    ]
    if len(sortie) > 0:
    ...
    Genre = [
        middleHeadLeftBR.index(index)
        for index in middleHeadLeftBR
        if "Genre(s)" in str(index)
    ]
    if len(Genre) > 0:
        Game["Genre"] = (
            str(middleHeadLeftBR[Genre[0]]).split(": ")[1].replace("\xa0", "")
        )
    else:
        Game["Genre"] = ""
    # Ajout du teritoire
    ...
    Game["nbrTropheeTotal"] = int(
        str(middleHeadLeft.find("div", attrs={"id": "game-total-trophies"}))
        .split(">")[1]
        .split(" ")[0]
    )
    # Ajout du nbrTropheeOnline
    Game["nbrTropheeOnline"] = int(
        str(middleHeadLeft.find("div", attrs={"id": "game-total-online"}))
        .split(">")[1]
        .split(" ")[0]
    )
    # Ajout du nbrTropheeCacher
    Game["nbrTropheeCacher"] = int(
        str(middleHeadLeft.find("div", attrs={"id": "game-total-hidden"}))
        .split(">")[1]
        .split(" ")[0]
    )
    # Ajout du nbrDLC
    if type(middleHeadLeft.find("div", attrs={"id": "game-total-dlc"})) is not type(
        None
    ):
        Game["DLC"] = int(
            str(middleHeadLeft.find("div", attrs={"id": "game-total-dlc"}))
            .split(">")[1]
            .split(" ")[0]
        )
    else:
    ...
    TropheeProc = {}
    mainTropheeHead = TropheeList.findAll(
        "div", attrs={"class": "horizontal-game-block"}
    )
    if len(mainTropheeHead) > 0:
        for line in mainTropheeHead:
            if "DLC" in str(line):
                TropheeProcDLCNbr = {}
                TropheeProcDLCNum = int(
                    str(line.find("h3")).split(")")[0].split("n°")[1]
                )
                TropheeProcDLCNbr["TropheePlatine"] = int(
                    str(line.find("div", attrs={"class": "platine"}))
                    .split('platine">')[1]
                    .split("<")[0]
                )
                TropheeProcDLCNbr["TropheeGold"] = int(
                    str(line.find("div", attrs={"class": "gold"}))
                    .split('gold">')[1]
                    .split("<")[0]
                )
                TropheeProcDLCNbr["TropheeSilver"] = int(
                    str(line.find("div", attrs={"class": "silver"}))
                    .split('silver">')[1]
                    .split("<")[0]
                )
                TropheeProcDLCNbr["TropheeBronze"] = int(
                    str(line.find("div", attrs={"class": "bronze"}))
                    .split('bronze">')[1]
                    .split("<")[0]
                )
                ...
                TropheeProcDLCTropheeTemp = TropheeList.find(
                    "div", attrs={"id": "guide-main-content-" + str(TropheeProcDLCNum)}
                )
                TropheeProcDLCTropheeTempline = TropheeProcDLCTropheeTemp.findAll(
                    "div", attrs={"class": "guide_line"}
                )
                for line in TropheeProcDLCTropheeTempline:
                    TropheeProcDLCTrophee["nom"] = (
                        str(line).split('name="')[1].split('"')[0]
                    )
                    TropheeProcDLCTrophee["trophee"] = (
                        str(line).split('niveau="')[1].split('"')[0]
                    )
                    TropheeProcDLCTrophee["pourcentage"] = float(
                        str(line).split('pourcj="')[1].split('"')[0]
                    )
                    TropheeProcDLCTrophee["image"] = (
                        str(line)
                        .split("<img class=")[1]
                        .split('src="')[1]
                        .split('"')[0]
                    )
                    TropheeProcDLCTrophee["description"] = (
                        str(line)
                        .split('guide_trophy_description">')[1]
                        .split("<")[0]
                        .replace("\n", "")
                    )
                ...
                TropheeProcMainNbr = {}
                TropheeProcMainNbr["TropheePlatine"] = int(
                    str(line.find("div", attrs={"class": "platine"}))
                    .split('platine">')[1]
                    .split("<")[0]
                )
                TropheeProcMainNbr["TropheeGold"] = int(
                    str(line.find("div", attrs={"class": "gold"}))
                    .split('gold">')[1]
                    .split("<")[0]
                )
                TropheeProcMainNbr["TropheeSilver"] = int(
                    str(line.find("div", attrs={"class": "silver"}))
                    .split('silver">')[1]
                    .split("<")[0]
                )
                TropheeProcMainNbr["TropheeBronze"] = int(
                    str(line.find("div", attrs={"class": "bronze"}))
                    .split('bronze">')[1]
                    .split("<")[0]
                )
                ...
                TropheeProcMainTropheeTemp = TropheeList.find(
                    "div", attrs={"id": "guide-main-content-0"}
                )
                TropheeProcMainTropheeTempline = TropheeProcMainTropheeTemp.findAll(
                    "div", attrs={"class": "guide_line"}
                )
                for line in TropheeProcMainTropheeTempline:
                    TropheeProcMainTrophee["nom"] = (
                        str(line).split('name="')[1].split('"')[0]
                    )
                    TropheeProcMainTrophee["trophee"] = (
                        str(line).split('niveau="')[1].split('"')[0]
                    )
                    TropheeProcMainTrophee["pourcentage"] = float(
                        str(line).split('pourcj="')[1].split('"')[0]
                    )
                    TropheeProcMainTrophee["image"] = (
                        str(line)
                        .split("<img class=")[1]
                        .split('src="')[1]
                        .split('"')[0]
                    )
                    TropheeProcMainTrophee["description"] = (
                        str(line)
                        .split('guide_trophy_description">')[1]
                        .split("<")[0]
                        .replace("\n", "")
        ...
        for line in TropheeProcMainTropheeTempline:
            ...
            TropheeProcMainTrophee["image"] = (
                str(line).split("<img class=")[1].split('src="')[1].split('"')[0]
            )
            TropheeProcMainTrophee["description"] = (
                str(line)
                .split('guide_trophy_description">')[1]
...
# ----------------------------------------------------------------
def getNombreJeuxRechercher(
    console="ps4", order="proche", direction="ASC", qquery="", letters=""
) -> int:
    headers = {
        "Host": "www.psthc.fr",
        "User-Agent": "Mozilla/5.0",
...
def getImportantLinks(url) -> dict:
    ...
    for linkItem in allLinksClear:
        if "guide-trophees" in str(linkItem):
            link["guideURL"] = (
                "https://www.psthc.fr" + str(linkItem).split('href="')[1].split("?")[0]
            )
